...
public class UnMuteCommand {
    ...
    public static void unmute(CommandSender sender, @Param(name="target") PunishmentTarget target, @Param(name="reason", wildcard=true) String reason) {
        target.resolveUUID(uuid -> {
            ...
            RequestResponse response = Hydrogen.getInstance().getPunishmentHandler().pardon(uuid, senderUUID, Punishment.PunishmentType.MUTE, reason);
    ...
    private static FancyMessage getStaffMessage(CommandSender sender, Player bukkitTarget, String correctedTarget, String reason) {
        ...
        staffMessage.then("un-muted by ").color(ChatColor.GREEN).tooltip(ChatColor.YELLOW + "Reason: " + ChatColor.RED + reason).then(senderName).tooltip(ChatColor.YELLOW + "Reason: " + ChatColor.RED + reason).then(".").color(ChatColor.GREEN).tooltip(ChatColor.YELLOW + "Reason: " + ChatColor.RED + reason);
        return staffMessage;
    }
}
